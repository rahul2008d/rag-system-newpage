...
uploaded = st.file_uploader(
    "Upload PDFs/TXTs", type=["pdf", "txt"], accept_multiple_files=True
)
...
query = st.text_input(
    "Question", placeholder="Ask something about the uploaded docs…"
)
ask = st.button("Ask", icon="💬")
...
for turn in reversed(st.session_state.chat):
    ...
    with st.expander(f"Sources (confidence={turn['score']:.3f})"):
        if not turn["sources"]:
